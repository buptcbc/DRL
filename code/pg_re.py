...
def concatenate_all_ob(trajs,pa):
    ...
    all_ob = np.zeros(
        (timesteps_total, 1, pa.network_input_height, int(pa.network_input_width)),
...
def concatenate_all_ob_across_examples(all_ob,pa):  # different test examples
    num_ex = len(all_ob)
    total_samp =0
    for i in range(num_ex):
        total_samp += all_ob[i].shape[0]
    print(type(total_samp),type(pa.network_input_height),type(pa.network_input_width))

    all_ob_contact = np.zeros(
        (total_samp,1,pa.network_input_height,int(pa.network_input_width)),
        dtype= theano.config.floatX
    )
...
def process_all_info(trajs):
    ...
    for traj in trajs :
        ...
        finish_time.append(np.array([traj['info'].record[i].end_time for i in range(len(traj['info'].record))]))
        job_len.append(np.array([traj['info'].record[i].job_mec_len for i in range(len(traj['info'].record))]))
...
def plot_lr_curve(output_file_prefix,max_rew_lr_curve,mean_rew_lr_curve,slow_down_lr_curve,
    ...
    ax.plot(mean_rew_lr_curve,linewidth = 2,label='PGMean')
...
def get_traj_worker(pg_learner,env,pa,result):
    ...
    #print("all_action", np.dtype(all_action[0]), "all_eprews", np.dtype(all_eprews[0]))
...
def launch(pa,pg_resume=None,render = False,end='no_new_job'):
    ...
    nw_len, net_len, nw_size = job.generate_work_sequence(pa,seed=42)

    for ex in range(pa.num_ex):
        print("prepare env")
        env = environment.environment(pa,nw_mec_len_seqs=nw_len,nw_net_len_seqs=net_len,nw_size_seqs=nw_size,
                                  seed=42,render=False,end=end)
        env.seq_no = ex
        envs.append(env)
    ...
    accums = init_accum(pg_learners[pa.batch_size])

    # --------------------------------------
    print("Preparing for reference data...")
    # --------------------------------------

    ref_discount_rews, ref_slow_down = slow_down_cdf.launch(pa, pg_resume=None, render=False, plot=False,
                                                           end=end)
    mean_rew_lr_curve = []
    max_rew_lr_curve = []
    slow_down_lr_curve = []
    ...
    for iteration in range(1,pa.num_epochs):
        ...
        for ex in range(pa.num_ex):
            ...
            if ex_counter >= pa.batch_size or ex == pa.num_ex - 1:
                ...
                #print('all_ob.shape', type(all_ob), all_ob.shape)
                #print('all_action', type(all_action), all_action.shape)
                #print('all_adv', type(all_adv), all_adv.shape)
        ...
        # assemble gradients
        grads = grads_all[0]
        # print(type(grads_all),len(grads_all))
        for i in range(1, len(grads_all)):
            for j in range(len(grads)):
                grads[j] += grads_all[i][j]
        # print(sum(np.array(grads)-np.array(grads_all)))
        ...
        if iteration % pa.output_freq == 0:
            ...
            slow_down_cdf.launch(pa, pa.output_filename + '_' + str(iteration) + '.pkl',
                                 render=False, plot=True, end=end)
            pa.unseen = False
...
def main():
    ...
    #pg_resume = 'data/tmp_net_file_10.pkl'
    pg_resume = None
    render = True
